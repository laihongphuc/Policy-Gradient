"""
using mean reward for baseline
"""
import random 
import math 
...
def reward_to_go(batch_reward):
    """
    Compute reward to go of a trajectory
    Args:
        - batch_reward: reward of a trajectory
    Returns:
        - rtg: reward to go of that trajectory
    """
    rtg = [0 for _ in range(len(batch_reward))]
    for i in range(len(batch_reward)-1, -1, -1):
        current_reward = batch_reward[i]
        if i == len(batch_reward) - 1:
            rtg[i] = current_reward 
        else:
            rtg[i] = rtg[i+1] + current_reward 
    return rtg
...
def train_one_epoch(env, network, optimizer, batch_size = 5000,render=False):
    ...
    while True:
        ...
        iter += 1
        if done:
            total_reward += reward_to_go(batch_reward)
    ...
    total_reward = torch.as_tensor(total_reward, dtype=torch.float32) - torch.mean(torch.as_tensor(reward_history))
    # forward pass
